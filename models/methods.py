...
def add_contact(args):
    ...
    while True:
        ...
        if phone.isnumeric() and len(phone) == 10:
            ...
            print(
                "[red]Invalid phone number. Please enter a [/red][bold yellow]10-digit numeric phone number.[/bold yellow]"
            )
...
def edit_phone(name):
    ...
    while True:
        try:
            phone_choice = int(
                input(
                    "Виберіть номер телефону для редагування (або введіть 0 для додавання нового): "
                )
            )
    ...
    if phone_choice == 0:
        ...
        try:
            ...
            print(
                "Невірний формат номеру телефону. Будь ласка, введіть правильний номер."
            )
    else:
        new_phone = input("Введіть новий номер телефону: ")
        try:
...
def edit_email(name):
    ...
    while True:
        try:
            email_choice = int(
                input(
                    "Виберіть номер електронної адреси для редагування (або введіть 0 для додавання нової): "
                )
            )
    ...
    if email_choice == 0:
        ...
        try:
            ...
            print(
                "Невірний формат електронної адреси. Будь ласка, введіть правильний email."
            )
    else:
...
def edit_birthday(name):
    ...
    if current_contact.add_birthday(new_birthday):
        ...
        print(
            "Failed to update birthday. Please enter a valid date format (DD.MM.YYYY)."
        )
...
def validate_contact():
    ...
    name_to_edit = prompt(
        "Оберить ім'я контакту із списку вишче: ",
        completer=completion.WordCompleter(contacts),
    )
...
def delete_note(name):
    note_to_delete = input(
        "Обери номер нотатки яку треба видалити, для видалення усіх нотаток введить команду all: "
    )
...
def show_all():
    ...
    for contact_name, contact in new_book.data.items():
        ...
        emails = (
            ", ".join([email.value for email in getattr(contact, "emails", [])])
            if hasattr(contact, "emails")
            else ""
        )
